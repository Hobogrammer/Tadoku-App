...
LANGREGEX = /#(fr|de|es|en|ko|th |zh|it|nl|pl|el|ru|eo|sv|he|nn|nb|la|hu|jp|fi)/i
	...
	def self.processor(request,subreq,client)
		unless !subreq.scan(/#undo/i).empty? #this is what happens when you tack on an important fucntion last.
			...
			db_update(request,new_read,medium,language,dr,reps)
		else
	...
	def self.db_update(req,count,med,lang,double,rep)
		...
		Update.new(:user_id => user.id, :newread => count, :medium => med, :lang => lang, :dr => double, :repeat => rep, :round_id => ApplicationHelper::curr_round,:recpage => total )
		user.rounds.find_by_round_id(ApplicationHelper::curr_round).update_attributes(:pcount => new_total)
	end
	...
	def self.undo(request,round,client)
		...
		old_total = Update::undo(user,round)
		user.rounds.find_by_round_id(round).update_attributes(:pcount => old_total)
